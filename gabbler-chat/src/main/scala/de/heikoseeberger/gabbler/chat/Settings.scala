...
final class Settings(system: ExtendedActorSystem) extends Extension {
  ...
  object chatApi {
    ...
    val address: String = config.getString("gabbler-chat.chat-api.address")

    val port: Int = config.getInt("gabbler-chat.chat-api.port")
  }

  object userRepository {
    val userEvents: Uri = Uri(
      config.getString("gabbler-chat.user-repository.user-events")
    )
  }
