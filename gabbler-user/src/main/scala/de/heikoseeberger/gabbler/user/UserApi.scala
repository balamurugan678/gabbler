...
object UserApi {
  ...
  def props(address: String, port: Int): Props =
    Props(new UserApi(address, port))
...
final class UserApi(address: String, port: Int)
    extends Actor
    with ActorLogging {
  ...
  Http(context.system).bindAndHandle(UserApi(), address, port).pipeTo(self)
